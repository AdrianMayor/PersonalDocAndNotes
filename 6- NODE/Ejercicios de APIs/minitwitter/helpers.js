...
const generateError = (message, code) => {
    ...
    err.statusCode = code; // Nos inventamos un nombre de propiedad para err donde alojaremos el codigo de error.
    return err;
};
...
const deletePhoto = async (imgName) => {
    try {
        ...
        try {
            // Intentamos acceder al archivo con la imagen mediante el metodo acces de fs. Este metodo genera un error si no es posible acceder al archivo
            await fs.access(imgPath);
        } catch {
            // Si salta error quiere decir que la imagen no existe asi que hacemois un return y finalizamos la funcion
            return false;
        }
